...
export default function HorizontalLinearStepper() {
  ...
  return (
    <Box sx={{ width: '100%',color:'#ffffff' }}>
      <Stepper color='#ffffff' activeStep={activeStep}>
        {steps.map((label, index) => {
          const stepProps = {};
          const labelProps = {};
          if (isStepOptional(index)) {
            labelProps.optional = (
              <Typography color='#ffffff' variant="caption">Optional</Typography>
            );
          }
          ...
          return (
            ...
            <StepLabel {...labelProps}>
              <Typography sx={{ color: '#ffffff' }} {...labelProps}>{label}</Typography>
            </StepLabel>
          </Step>
          );
        })}
